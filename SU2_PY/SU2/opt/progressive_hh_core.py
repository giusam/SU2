...
class HHLevel:
    def __init__(
        ...
        # Keep track of the original reference mesh independently from the
        # mesh used at the current level.  This enables the alternative
        # refinement mode where every refined level is rebuilt from the
        # initial mesh while the old HH coefficients are retained.
        self.initial_mesh_source = (
            initial_mesh_source if initial_mesh_source is not None else mesh_source
        )
        # Values written as DV_VALUE_NEW in the level config.  These are the
        # config-space values used by SU2; write_level_config falls back to
        # zero when this is None.
...
def get_progressive_hh_options(config):
    ...
    refine_state_mode = str(
        config.get("PROGRESSIVE_HH_REFINE_STATE", "DEFORMED_MESH_ZERO_DV")
    ).upper()
    refine_state_aliases = {
        "ZERO": "DEFORMED_MESH_ZERO_DV",
        "ZERO_DV": "DEFORMED_MESH_ZERO_DV",
        "DEFORMED_ZERO": "DEFORMED_MESH_ZERO_DV",
        "DEFORMED_MESH": "DEFORMED_MESH_ZERO_DV",
        "KEEP": "INITIAL_MESH_KEEP_DV",
        "KEEP_DV": "INITIAL_MESH_KEEP_DV",
        "KEEP_COEFFICIENTS": "INITIAL_MESH_KEEP_DV",
        "INITIAL_MESH": "INITIAL_MESH_KEEP_DV",
        "INITIAL_MESH_KEEP_COEFFICIENTS": "INITIAL_MESH_KEEP_DV",
    }
    refine_state_mode = refine_state_aliases.get(refine_state_mode, refine_state_mode)
    ...
    allowed_refine_state_modes = (
        "DEFORMED_MESH_ZERO_DV",
        "INITIAL_MESH_KEEP_DV",
    )
    if refine_state_mode not in allowed_refine_state_modes:
        raise ValueError(
            "Invalid PROGRESSIVE_HH_REFINE_STATE "
            f"{refine_state_mode!r}; allowed values are {allowed_refine_state_modes}"
        )
