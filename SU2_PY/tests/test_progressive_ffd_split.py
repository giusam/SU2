...
def test_su2_def_moves_offset_endpoints_but_keeps_airfoil_edges_fixed(
    tmp_path,
    blending,
    bspline_orders,
):
    ...
    config = tmp_path / "offset_endpoints.cfg"
    config.write_text(
        "\n".join(
            [
                "SOLVER= EULER",
                ...
                "MESH_FILENAME= dual.su2",
                "MESH_FORMAT= SU2",
                "MESH_OUT_FILENAME= offset_endpoints_out",
                "MARKER_EULER= ( AIRFOIL )",
                "MARKER_FAR= ( FARFIELD )",
                "MARKER_PLOTTING= ( AIRFOIL )",
                "MARKER_MONITORING= ( AIRFOIL )",
                ...
                "DV_MARKER= ( AIRFOIL )",
                "DV_PARAM= ( UPPER_BOX, 0, 1, 0.0, 1.0 ); "
                "( UPPER_BOX, 4, 1, 0.0, 1.0 ); "
                "( LOWER_BOX, 0, 0, 0.0, -1.0 ); "
                "( LOWER_BOX, 4, 0, 0.0, -1.0 )",
                "DV_VALUE= 0.002, 0.002, 0.002, 0.002",
    ...
    deformed = read_su2_mesh(tmp_path / "offset_endpoints_out.su2")["points"]
    for edge_id in (0, 4):
        assert deformed[edge_id] == pytest.approx(original[edge_id], abs=1.0e-12)
    for upper_id in (1, 3):
        assert deformed[upper_id][1] > original[upper_id][1]
    for lower_id in (5, 7):
        assert deformed[lower_id][1] < original[lower_id][1]
